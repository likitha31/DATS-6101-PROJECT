...
It is seen that the majority of census tracts fall in a lower range, with a clustering of hugher incomes in the coastal region in the middle of the state. 
...
correlation_matrix1 <- svi_select %>%
  select(income, EPL_POV150, EPL_HBURD, EPL_NOHSDP)
...
corrplot(correlation_matrix, method = "square", type = "upper", col = colorRampPalette((c("#B2182B", "#FDDBC7", "#2166AC")))(100))
...
```{r}
simple_model <- lm(RPL_THEMES ~ income + EPL_POV150 + EPL_HBURD + EPL_NOHSDP, data = predict_svi)
...
# Compare AIC values
cat("AIC for initial_model:", aic_simple_model, "\n")
cat("AIC for initial_model2:", aic_full_model, "\n")
