...
**Conclusion:** The results indicate that income (measured by median income of census tracts) is likely a good predictor of SVI risk scores, as it is statistically significant. 
...
The residuals appear to have a normal distribution, so now it is possible to assess them using VIF assessment to address multicolinearity. 
...
Assessing the AIC values of both models, it is seen that the AIC for the simple model is -13891.99 while the AIC for the full model is-18331.04. Therefore, the lower AIC is the full model. 
...
Both simple and full models have VIF values under 10, and only one value (income) in the full model is slightly above 5. This is still within the range of acceptable values. 
