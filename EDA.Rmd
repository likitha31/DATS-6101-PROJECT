...
ca_svi_sf_clean <- subset(ca_svi_sf, E_MINRTY != -999)
#normalize the data 
ca_svi_sf_clean$E_MINRTY <- (ca_svi_sf_clean$E_MINRTY - min(ca_svi_sf_clean$E_MINRTY)) / (max(ca_svi_sf_clean$E_MINRTY) - min(ca_svi_sf_clean$E_MINRTY))


color_pal <- colorNumeric(
  palette = "RdPu", domain = ca_svi_sf_clean$E_MINRTY
)

perminority_map <- leaflet() %>%
  addTiles() %>%
  addPolygons(data = ca_svi_sf_clean,
              fillColor = ~color_pal(E_MINRTY),
              fillOpacity = 0.5,
              color = "black",
              weight = 1,
              popup = ~as.character(E_MINRTY)) %>%
  setView(lng = -119.417931, lat = 36.778261, zoom = 6)  # Set the initial view
...
### E_AGE65 Map (Persons aged 65 and older estimate, 2016-2020 ACS)

```{r}
# Remove rows with -999 in the E_AGE65 column

ca_svi_sf_clean <- subset(ca_svi_sf, E_AGE65 != -999)


color_pal <- colorNumeric(
  palette = "RdPu", domain = ca_svi_sf_clean$E_AGE65
)

estage_map <- leaflet() %>%
  addTiles() %>%
  addPolygons(data = ca_svi_sf_clean,
              fillColor = ~color_pal(E_AGE65),
              fillOpacity = 0.5,
              color = "black",
              weight = 1,
              popup = ~as.character(E_AGE65)) %>%
  setView(lng = -119.417931, lat = 36.778261, zoom = 6)  # Set the initial view
...
```{r}
boxplot(CA_SVI$EPL_POV150, CA_SVI$EPL_UNEMP, CA_SVI$EPL_HBURD, CA_SVI$EPL_NOHSDP, CA_SVI$EPL_UNINSUR, RPL_THEME1)
