...
# Preparing the independent variable (X) and the dependent variable (y)
y <- svi_all_counties_cases$confirmed_cases
X <- svi_all_counties_cases[, c('EP_NOHSDP','EP_AGE17','EP_MUNIT','EP_NOVEH','EP_GROUPQ')]
# Splitting the data into train and test data
set.seed(42)  # For reproducibility
train_indices <- createDataPartition(y, p = 0.8, list = FALSE)
X_train <- X[train_indices, ]
...
# Initiate the model
lm_model2 <- lm(y_train ~ ., data = cbind(y_train, X_train))

# Predictions on the test set
y_predict <- predict(lm_model2, newdata = X_test)
summary(lm_model2)
# Score (R-squared)
