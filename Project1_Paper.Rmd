...
Here it is seen that there is a large number of missing values, represented by `-999`, but the number of missing values does not equal the number of outliers identified. Thus, for the purposes of the analysis the missing values values were removed, but the outliers were not removed so it would be possible to identify any particular at risk census tracts in the analysis. These values were removed from further analysis case by case.  
...
### Maps
It is interesting to examine the spatial distribution of this dataset, given that it looks across census tracts, a geographic scale. This portion of the analysis examines the spatial distribution of SVI and demographic variables. **A high level overview of the results is provided on each tab. These maps also help visualize the correlation, or lack thereof, shown in the other types of data visaulization completed for the data analysis.** *For example, looking at the distirubtion of age can help us understand why age is not a great influence on SVI.*
...
This was done at the census and county level scale. For the purposes of rendering HTML, the county level mapping was omitted. No new insights were gained and census tract data is at a finer scale, so it is more useful for analysis. Additionally, leaflet interactive maps were produced but not able to be rendered to HTML at such a scale at this time. Future applications could include the development of dashboards with interactive mapping, but this tool already exists, created by the [CDC](https://www.atsdr.cdc.gov/placeandhealth/svi/interactive_map.html). 
...
The spatial distribution of `Overall SVI` is shown here, with areas in the center of the state showing up as higher in vulnerability. In depth research into the demographics of these counties can be viewed in the Charts tab. 
...
A similar spatial distribution is seen in `Theme 1` as `Overall SVI`, but toggling back and forth can be helpful to identify the differences in the maps. 
...
This shows a slightly different distribution, with less high vulnerable tracts in the center, and more so concentrated in the Norther part of the state. It also appears there are less high risk `Theme2` tracts overall. 
...
Mapping `Theme 3` shows the distribution of Racial & Ethnic Minority groups in the state. This indicates that the groups that would be vulnerable under Theme 3 are located in the Southwest portion of the state. 
...
Interestingly, the `Theme 4` map shows a different distribution with not much apparent clustering, but rather a decent number of vulnerable tracts scattered throughout the state. 
...
This map and the map of `Theme 3` are very similar, as they both map the percentile of minority groups residing in those tracts, thus they will create the same output. 
...
Just as the results showed in the scatterplot analysis and correlation plot analysis, there is not too much correlation between Age and SVI. This is seen by a nearly universal distribution of the `Age 65+` variable across the state. 
...
The results of this map point to interesting results, identifying those tracts experiencing severe poverty. The tracts in the South would be interesting observations to look into as to why they differ so greatly from the rest. A concentration of higher vulnerability can be seen in the center of the state.
